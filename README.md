...
- **For-Xcode (Xcode Snippets)**: Contains code snippets that helps to increase the productivity of an engineer while interacting with Xcode.
  - **open-terminal.sh**: Script added under Xcode's custom `Behaviors` in `Settings` to open temrinal directly from Xcode using a shortcut `command` + `T` on mac keyboards.